...
def ablation_sampler_single_step(
    net, x_t, i, t_cur, t_next, class_labels=None, randn_like=torch.randn_like,
    num_steps=11, sigma_min=None, sigma_max=None, rho=7,
    solver='heun', discretization='edm', schedule='linear', scaling='none',
    epsilon_s=1e-3, C_1=0.001, C_2=0.008, M=1000, alpha=1,
    S_churn=0, S_min=0, S_max=float('inf'), S_noise=1,
):
    ...
    assert schedule in ['vp', 've', 'linear']
    assert scaling in ['vp', 'none']
    dist.print0(f"{solver} sampler steps: {num_steps}")
    ...
    # Define time steps in terms of noise level.
    step_indices = torch.arange(num_steps, dtype=torch.float64, device=x_t.device)
    if discretization == 'vp':
        orig_t_steps = 1 + step_indices / (num_steps - 1) * (epsilon_s - 1)
        sigma_steps = vp_sigma(vp_beta_d, vp_beta_min)(orig_t_steps)
    elif discretization == 've':
        ...
        u = torch.zeros(M + 1, dtype=torch.float64, device=x_t.device)
        alpha_bar = lambda j: (0.5 * np.pi * j / M / (C_2 + 1)).sin() ** 2
        for j in torch.arange(M, 0, -1, device=x_t.device): # M, ..., 1
            u[j - 1] = ((u[j] ** 2 + 1) / (alpha_bar(j - 1) / alpha_bar(j)).clip(min=C_1) - 1).sqrt()
        u_filtered = u[torch.logical_and(u >= sigma_min, u <= sigma_max)]
        sigma_steps = u_filtered[((len(u_filtered) - 1) / (num_steps - 1) * step_indices).round().to(torch.int64)]
    else:
    ...
    eps_l2_norm = (sum(l2_norms) / len(l2_norms))
    eps_l2_norm = np.array(eps_l2_norm).reshape([1])
    if str(sigma(t_hat)) in PRED_EPS_COR.keys():
        PRED_EPS_COR[str(sigma(t_hat))] = np.concatenate((PRED_EPS_COR[str(sigma(t_hat))], eps_l2_norm), axis=0)
    else:
        PRED_EPS_COR[str(sigma(t_hat))] = eps_l2_norm
    dist.print0(f"store eps L2-norm: {eps_l2_norm} at euler: {sigma(t_hat)}")
...
def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, device=torch.device('cuda'), **sampler_kwargs):
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        for batch_seeds in rank_batches:
            ...
            sampler_fn = ablation_sampler_single_step if have_ablation_kwargs else edm_single_step_sampler
